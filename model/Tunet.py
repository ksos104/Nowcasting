...
class Tunet(nn.Module):
    def __init__(self, in_channel, out_channel, hidden_channel=16):
        ...
        self.skip1 = nn.Conv2d(hidden_channel,hidden_channel,kernel_size=1)
        self.maxPool1 = nn.MaxPool2d(kernel_size=2, stride=2)
        self.encoder2 = encoder_block(hidden_channel*2, hidden_channel*4)
        self.skip2 = nn.Conv2d(hidden_channel,hidden_channel,kernel_size=1)
        self.maxPool2 = nn.MaxPool2d(kernel_size=2, stride=2)
        self.encoder3 = encoder_block(hidden_channel*4, hidden_channel*8)
        self.skip3 = nn.Conv2d(hidden_channel,hidden_channel,kernel_size=1)
    ...
    def forward(self, x):
        ...
        concat0 = torch.cat([encoded3, self.skip0(transposed0)], dim=1)
        decoded0 = self.decoder0(concat0)
        transposed1 = self.convT1(decoded0)
        concat1 = torch.cat([encoded2, self.skip1(transposed1)], dim=1)
        decoded1 = self.decoder1(concat1)
        transposed2 = self.convT2(decoded1)
        concat2 = torch.cat([encoded1, self.skip2(transposed2)], dim=1)
        decoded2 = self.decoder2(concat2)
        transposed3 = self.convT3(decoded2)
        concat3 = torch.cat([encoded0, self.skip3(transposed3)], dim=1)
        decoded3 = self.decoder3(concat3)

        output = self.s(decoded3)
